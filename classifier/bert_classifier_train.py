...
def train(model_name, train_data, val_data, learning_rate, epochs):
    ...
    train_dataloader = torch.utils.data.DataLoader(train_set, batch_size=24, shuffle=True, num_workers=0)
    ...
    for epoch_num in range(1, epochs + 1):
        ...
        train_log = f"EPOCH {epoch_num} TRAIN Loss: {(total_loss_train / len(train_data)):.6f} Accuracy: {(total_acc_train / len(train_data)):.6f} Recall: {recall_t:.4f} TP/FN: {tp_t}/{fn_t}"
        val_log = f"EPOCH {epoch_num} VAL Loss: {(total_loss_val / len(val_data)):.6f} Accuracy: {(total_acc_val / len(val_data)):.6f} Recall: {recall_v:.4f} Precision: {precision_v:.4f} TP/FN: {tp_v}/{fn_v}"
...
if __name__ == "__main__":
    ...
    train_path = os.path.join("..", "data", "cns_train_aug.csv")
    val_path = os.path.join("..", "data", "cns_val.csv")
    ...
    train('pubmed_abstract', train_data, val_data, LR, EPOCHS)
    #
